...
1. **Working tree clean?** → State: `Builder 3 session-close ready: wt3@<commit SHA>, clean`. Done.
2. **Uncommitted work passing gates?** (`cargo check` + `cargo clippy --all-targets` + `cargo test` all green) → commit with `[Builder]` prefix + specific files (NEVER `-A`/`.`). State: `Builder 3 session-close ready: wt3@<new commit SHA>`. Done.
3. **WIP that does NOT pass gates?** → `git stash push -m "wt3 WIP YYYY-MM-DD <one-line summary>" -- <specific files>`. State: `Builder 3 session-close ready: wt3@<last commit SHA>, stash: "<label>"`. Done.
